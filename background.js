...
Provide two fields:
- translation: the selected text translated into Russian — or, if the selected text is itself in Russian, into English. Render it in the OTHER language; never leave it in its original language. Expand abbreviations using the context. Translate only the text between the markers.
- notes: a short note (max ~50 words) that helps the reader truly understand it. Write the note in the SAME language as the selected text — if the selected text is in Russian, the note must be in Russian; if it is in English, in English. If it contains or alludes to a literary, biblical, historical, or cultural reference, a named work or person, a joke, an allusion, an idiom, or a double meaning — explain what it refers to and, if it is used ironically or rhetorically or as a joke, what the actual point is. If it quotes or names something famous, consider whether it is commonly misattributed or whether its modern usage differs from the original meaning, and say so. If the text is plain with no such reference, briefly clarify its meaning, nuance, or tone. Never leave notes empty.
